...
def transform(img):
    ...
    resize_img_tensor = torch.as_tensor(resize_img.transpose(2, 0, 1)).to('cuda:2')
...
def print_trainable_parameters(model):
    ...
    print(
        f"trainable params: {trainable_params} || all params: {all_param} || trainable%: {100 * 1024 / all_param:.6f}"
    )
...
# train data path
npz_tr_path = 'data/Npz_files/CT_Abd-Gallbladder/train'
work_dir = './work_dir_SVD_encoder_true_global_rank_1.0_v3_only_1x1_3x3_no_linear'
...
sam_model.image_encoder = svf_torch.resolver(sam_model.image_encoder,global_low_rank_ratio=1.0,  # no need to change
...
# Count the 1x1 convolutions in the model
# num_1x1_convolutions, count_layers = count_1x1_convolutions(sam_model)

# print("Number of 1x1 convolutions:", num_1x1_convolutions)
# print("Number of layers:", count_layers)

# num_3x3_convolutions, count_layers = count_3x3_convolutions(sam_model)

# print("Number of 3x3 convolutions:", num_3x3_convolutions)
# print("Number of layers:", count_layers)
...
optimizer = torch.optim.Adam(sam_model.parameters(), lr=1.25e-6, weight_decay=0)
